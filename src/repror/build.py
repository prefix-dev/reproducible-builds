...
from repror.util import calculate_hash, find_conda_build, get_recipe_name, move_file, run_command
...
def build_recipe(recipe_path, output_dir) -> Optional[BuildInfo]:
    ...
    return BuildInfo(recipe_path=str(recipe_path), pkg_hash=first_build_hash, output_dir=str(output_dir), conda_loc=str(new_file_loc))
...
def rebuild_package(conda_file, output_dir) -> Optional[BuildInfo]:
    ...
    return BuildInfo(recipe_path=str(conda_file), pkg_hash=first_build_hash, output_dir=str(output_dir), conda_loc=str(conda_file))




def build_remote_recipes(repo, build_dir, cloned_prefix_dir) -> dict[str, Optional[BuildInfo]]:
    repo_url = repo["url"]
