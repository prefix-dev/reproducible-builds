...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with open(
            f"build_info/{platform}_{previous_version}_{recipe_string}_build_info.json", "r"
        ) as f:
        ...
        with open(
            f"build_info/{platform}/{platform}_{previous_version}_{current_version}_{recipe_string}_rebuild_info.json",
            "w",
        ) as f:
            json.dump(rebuild_info, f)

        with open(
            f"build_info/{platform}/{platform}_{previous_version}_{recipe_string}_build_info.json",
            "w",
        ) as f:
